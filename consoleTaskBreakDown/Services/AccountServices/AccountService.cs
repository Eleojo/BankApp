...
namespace BankApp.Services.AccountServices
{
    internal class AccountService : IAccountService
    {
        ...
        public AccountService(IFileManager fileManager)
        {
        ...
        public void OpenAccount(Guid userId)
        {
            ...
            //var accounts =  _fileManager.ReadAccountsFromFile(_accountsFilePath);
            ...
            var accountExists = (from u in _users
                                 join a in _accounts on u.Id equals a.userId
            ...
            if (accountExists)
            {
                var existingAccount = _accounts.SingleOrDefault(a => a.userId == userId && a.accountType.Equals(accountType));
            ...
            else
            {
                ...
                var newAccount = new Account(userId, initialAccountBalance, accountNumber, accountType);

                // add newAccount to accounts list and write accounts list to "Accounts.txt" file.
                _accounts.Add(newAccount);

                _fileManager.WriteAccountsToFile(_accounts, _accountsFilePath);
        ...
        private bool IsUniqueAccountNumber(string accountNumber)
        {

            //var accounts =  _fileManager.ReadAccountsFromFile(_usersFilePath);
            return !_accounts.Any(a => a.accountNumber == accountNumber);
            
        }

        public void DisplayAccountInfo(User sessionUser)
        {
            //BankAppDbContext db = new BankAppDbContext();
            var users =  _fileManager.ReadUsersFromFile(_usersFilePath);
           var accounts =  _fileManager.ReadAccountsFromFile(_accountsFilePath);// reload updated accounts
